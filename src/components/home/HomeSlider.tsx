...
const imageList: string[] = Object.entries(
    import.meta.glob<{ default: string }>('/src/assets/imgs/slider/*', { eager: true })
)
...
const captions: Caption[] = [
    {
        ...
        text: "Organizza luoghi, idee e momenti in un’unica agenda.",
        highlight: "Agende tematiche a colori"
    },
    {
        title: "Pianificazione Intuitiva",
        text: "Eventi e note personalizzabili in pochi tap.",
        highlight: "Aggiorna velocemente"
    },
    {
        title: "Colleziona Ricordi",
        text: "Valuta esperienze e tieni traccia di tutto.",
        highlight: "Tag intelligenti"
    },
    {
        title: "Promemoria che contano",
        text: "Alert puntuali quando serve davvero.",
        highlight: "Email reminder"
    }
];
...
export function HomeSlider() {
    ...
    const scrollToFeature = (e: React.MouseEvent) => {
        e.preventDefault();
        document.getElementById("feature-spotlight")?.scrollIntoView({
            behavior: "smooth",
            block: "start"
        });
    };
    ...
    const onTouchEnd = (e: React.TouchEvent) => {
        ...
        if (Math.abs(dx) > 40) {
            setIdx((prev) =>
                dx < 0 ? (prev + 1) % imageList.length : (prev - 1 + imageList.length) % imageList.length
            );
        }
        ...
        >
            <div className="slider-track">
                {imageList.map((src, i) => (
                    <div key={i} className={`slider-frame ${i === idx ? "active" : ""}`}>
                        <img src={src} alt={`Slide ${i + 1} - ${captions[i % captions.length]?.title}`} />
            ...
            <div className="slider-content">
                <div className="slider-card">
                    ...
                    {cap.highlight && (
                        <div className="slider-pill">🏷️ {cap.highlight}</div>
                    )}
